clear all;
testName = 'A1-BCFC-Same';
outFolder = ('C:\Users\u0137276\Desktop\epDATA\expVP\decodingMatlab\A1\decoding45\BCFC\');
...
trainL = {{train{2} train{8}}, {train{3} train{7}}, {train{4} train{6}}}; 
testL = {{test{2} test{8}}, {test{3} test{7}}, {test{4} test{6}}}; % same
% trainL = {{train{2} train{6}}, {train{1} train{5}}, {train{4} train{8}}}; % max
% testL = {{test{2} test{6}}, {test{1} test{5}}, {test{4} test{8}}};
...
%%     
    ...
    hold all
    set(gca, 'FontSize', 12, 'FontWeight','bold', 'yLim', [0.4, 1], 'yTick', [0.4:0.1:1], 'xLim', [0, 4], 'xTick', [1: 3], 'xticklabel', {xValues{:}});
    ylabel('Accuracy', 'FontSize', 12, 'FontWeight','bold');
    yline(0.5, '--');
    h = title([testName, '-', nm1{1}]);
    set(h, 'FontSize', 14, 'FontWeight', 'bold');
    figName = fullfile(outFolder, [testName, '.svg']);
    saveas(fig, figName);
    close all;
end
